...
| **`--dangerously-skip-permissions`** | This flag is safe inside the sandbox. It tells Claude Code to skip its own permission prompts (tool approvals), which makes sense because the container itself is the security boundary. |
...
**Practical risk**: Claude Code running `curl` or `wget` to arbitrary URLs. This is the same risk as running Claude Code on your host Mac with `--dangerously-skip-permissions`. The sandbox adds filesystem isolation on top, which is the main value.
